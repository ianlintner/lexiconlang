...
export function mix(parent: bigint, label: string): bigint {
  return splitmix64(parent ^ fnv1a64(label));
}

export function seedToBigInt(seed: string | number | bigint): bigint {
  if (typeof seed === "bigint") return seed & MASK_64;
  if (typeof seed === "number") return splitmix64(BigInt(Math.trunc(seed)) & MASK_64);
  return fnv1a64(seed);
}
