...
const humanGiven = pickOf(
  markov(humanModelMale),
  markov(humanModelFemale),
);

const humanSurname = oneOf(
  "Voss", "Kalra", "Okafor", "Marston", "Han", "Reyes", "Petrov",
  "Hayashi", "Chowdhury", "Nakamura", "Singh", "Beckett", "Drago",
...
const role = oneOf(
  "Captain", "First Officer", "Pilot", "Engineer", "Medic", "Gunner",
  "Navigator", "Comms Officer", "Scientist", "Quartermaster", "Marine",
);
...
export const crewMember: Generator<CrewMember> = {
  id: "scifi.crewMember",
  generate(ctx: Context) {
...
export interface StarSystem {
  name: string;
  star: string;
  planets: { name: string; type: string }[];
  faction: string;
}

const planetType = weightedList<string>(
  {
