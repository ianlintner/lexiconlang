...
describe("sample-output integration", () => {
  it("produces a deterministic samples.txt artifact", () => {
    ...
    out.push(`seed: ${SEED}\n`);
    out.push(
      `generated: deterministic — re-running this test must produce identical content\n`,
    );
    ...
    const fantasyNames = repeat(fantasyFullName, 50).generate(
      ctx.child("fantasy.names"),
    );
    out.push(
      bullet(
        fantasyNames.map(
    ...
    const taverns = repeat(fantasy.generators.tavernName, 30).generate(
      ctx.child("fantasy.taverns"),
    );
    out.push(bullet(taverns));

    out.push(header("FANTASY · 20 cities"));
    const cities = repeat(fantasy.generators.cityName, 20).generate(
      ctx.child("fantasy.cities"),
    );
    out.push(bullet(cities));

    out.push(header("FANTASY · 15 villages"));
    const villages = repeat(fantasy.generators.villageName, 15).generate(
      ctx.child("fantasy.villages"),
    );
    out.push(bullet(villages));

    out.push(header("FANTASY · 15 settlements (composite)"));
    const settlements = repeat(settlement, 15).generate(
      ctx.child("fantasy.settlements"),
    );
    ...
    const factions = repeat(fantasy.generators.factionName, 20).generate(
      ctx.child("fantasy.factions"),
    );
    out.push(bullet(factions));

    out.push(header("FANTASY · 10 cults"));
    const cults = repeat(fantasy.generators.cultName, 10).generate(
      ctx.child("fantasy.cults"),
    );
    out.push(bullet(cults));

    out.push(header("FANTASY · 15 mountains / forests / rivers"));
    const landmarks = repeat(fantasy.generators.landmarkName, 15).generate(
      ctx.child("fantasy.landmarks"),
    );
    out.push(bullet(landmarks));

    out.push(header("FANTASY · 15 weapons + 10 armor"));
    const weapons = repeat(fantasy.generators.weaponName, 15).generate(
      ctx.child("fantasy.weapons"),
    );
    const armor = repeat(fantasy.generators.armorName, 10).generate(
      ctx.child("fantasy.armor"),
    );
    out.push(bullet([...weapons, ...armor]));

    out.push(header("FANTASY · 15 dragons"));
    const dragons = repeat(fantasy.generators.dragon, 15).generate(
      ctx.child("fantasy.dragons"),
    );
    out.push(bullet(dragons));

    out.push(header("FANTASY · 10 quest hooks"));
    const hooks = repeat(fantasy.generators.questHook, 10).generate(
      ctx.child("fantasy.hooks"),
    );
    ...
    const ships = repeat(scifi.generators.shipName, 20).generate(
      ctx.child("scifi.ships"),
    );
    ...
    const corps = repeat(scifi.generators.megacorpName, 15).generate(
      ctx.child("scifi.corps"),
    );
    out.push(bullet(corps));

    out.push(header("SCI-FI · 10 weapons"));
    const scifiWeapons = repeat(scifi.generators.weaponName, 10).generate(
      ctx.child("scifi.weapons"),
    );
    ...
    const mCities = repeat(modern.generators.cityName, 20).generate(
      ctx.child("modern.cities"),
    );
    out.push(bullet(mCities));

    out.push(header("MODERN · 20 streets"));
    const streets = repeat(modern.generators.streetName, 20).generate(
      ctx.child("modern.streets"),
    );
    out.push(bullet(streets));

    out.push(header("MODERN · 20 companies"));
    const companies = repeat(modern.generators.companyName, 20).generate(
      ctx.child("modern.companies"),
    );
    out.push(bullet(companies));

    out.push(header("MODERN · 15 bands + 15 songs + 15 books"));
    const bands = repeat(modern.generators.bandName, 15).generate(
      ctx.child("modern.bands"),
    );
    const songs = repeat(modern.generators.songTitle, 15).generate(
      ctx.child("modern.songs"),
    );
    const books = repeat(modern.generators.bookTitle, 15).generate(
      ctx.child("modern.books"),
    );
    out.push("Bands:\n" + bullet(bands));
    out.push("Songs:\n" + bullet(songs));
    out.push("Books:\n" + bullet(books));
