...
```ts
import { dwarvish, elvish, orcish, halfling, draconic } from "@lexiconlang/fantasy";
```
...
```ts
import {
  elvenMaleName, elvenFemaleName,
  dwarvenMaleName, dwarvenFemaleName,
  humanMaleName, humanFemaleName,
  orcishName, halflingName, draconicName,
...
```ts
import {
  npc, fullName, settlementName, mountainName,
  forestName, riverName, cityName, villageName,
} from "@lexiconlang/fantasy";
```
